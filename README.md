...
| --- | --- | --- | --- |
| **通用 YOLO** | `models/yolo26s.pt`，COCO 公开预训练（AGPL-3.0） | person / cell phone 检测，为离岗、在岗、人流、玩手机、抽烟、闯入提供候选与跟踪 | `YOLO_IMGSZ=640`、`YOLO_CONFIDENCE=0.35`、`YOLO_IOU=0.5`、CPU/GPU 由 `YOLO_DEVICE` 决定；在岗/离岗人员再叠加 `confidence≥0.45`，人员检测框只要与岗位 ROI 有任意交叠即视为在岗 |
