...
| 离岗 | 排班内检测置信度不低于 0.30 的人员框是否与岗位 ROI 相交；持续无人达到当前时段的 `off_duty_seconds` 后由外部 VLM 终审当前岗位区域 | VLM 明确确认后产生 `normal` 告警 |
...
玩手机和吸烟共用一次单模型联合 VLM 请求。两项同时确认时分别生成告警，但共用原始证据帧。
