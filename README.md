...
岗位 ROI 和禁区至少需要 3 个归一化坐标点。烟火、区域入侵和黑屏属于全天安全模式，不受普通排班限制。
...
| 离岗 | 排班内检测置信度不低于 0.30 的人员框是否与岗位 ROI 相交；持续无人达到 `off_duty_seconds` 并考虑班次宽限 | `normal` 告警 |
| 在岗记录 | 人员框与岗位 ROI 有任意交叠即判定在岗 | 仅记录 |
| 人流 | YOLO 人员检测和独立跟踪，新轨迹稳定确认后累计进入画面人次 | 仅统计 |
| 玩手机 | 外部 VLM 分析当前帧，要求明确操作或注视手机 | `normal` 告警 |
| 吸烟 | 外部 VLM 分析持烟、吸食动作或可关联烟雾 | `normal` 告警 |
| 烟火 | 独立 YOLO，火焰与烟雾默认阈值均为 0.30；火焰连续 2 帧命中，烟雾最近 5 帧至少 3 帧命中 | `critical` 告警 |
...
人流模块基于稳定的新跟踪 ID 统计进入画面人次，展示今日总人流、当前画面人数、当日趋势、当前人数排名、今日人流排名以及逐摄像头统计。

人流统计直接复用通用 YOLO 的 Person 检测和 ByteTrack 跟踪结果，不会额外运行一套模型。新轨迹默认连续出现 3 帧后确认；从画面四周 10% 范围内建立的轨迹优先视为新进入人员。画面中央建立的轨迹需要额外稳定两帧且产生有效位移，以降低遮挡、漏检和 ID Switch 导致的重复统计。

同一 Track 生命周期只累计一次。Track 短暂丢失后，新 ID 会根据时间和归一化空间距离尝试继承旧 Track 的计数状态；系统启动、日期切换及摄像头断流恢复后默认启用 15 秒保护期，避免把画面内原有人员批量计为新客流。同一现实人员离开超过关联窗口后再次进入画面，会作为新的人次重新统计。

摄像头高级选项提供稳定确认帧数、边缘比例和人流 Debug 标注。Debug 开启后，实时预览显示 Track ID、首次出现位置、稳定帧数、计数状态、当前画面人数和今日累计人流。
