...
系统不提供公开注册。首次启动且数据库中没有账号时，必须设置 `ADMIN_USERNAME`、`ADMIN_DISPLAY_NAME` 和 `ADMIN_PASSWORD`，系统会创建首个管理员；密码需为 8-128 位。创建成功后可从运行环境移除明文 `ADMIN_PASSWORD`。

管理员可使用侧栏“账号管理”创建、启停、重置密码和删除普通用户。普通用户只能访问监控总览、人流报表和告警中心，可查看数据及手动推送告警，不能操作摄像头、清理数据或修改配置。登入会话连续闲置 8 小时后失效，修改或重置密码会注销该账号的所有会话。

HTTPS 部署请设置 `SECURE_COOKIES=true`，并通过 `ALLOWED_ORIGINS` 配置允许访问 WebSocket 的前端来源。
