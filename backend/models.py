...
class Analysis(Base):
    ...
    occupancy_status: Mapped[str | None] = mapped_column(String(30), index=True)
    off_duty_state: Mapped[str | None] = mapped_column(String(40), index=True)
    analysis_source: Mapped[str | None] = mapped_column(String(30), index=True)
    absence_started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    absence_elapsed_seconds: Mapped[int | None] = mapped_column(Integer)
