...
class AsyncPipeline:
    ...
    def review_invalid(self, camera_id, mode):
        ...
        if (
            mode != Mode.OFF_DUTY.value
            and (now - sampled).total_seconds() > self.runtime.settings.review_max_evidence_age_seconds
        ):
    ...
    async def _review(self, request):
        ...
        if (
            request.kind != "off_duty"
            and (utc_now() - datetime.fromisoformat(request.sampled_at)).total_seconds()
            > self.runtime.settings.review_max_evidence_age_seconds
        ):
