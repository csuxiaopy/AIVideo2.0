...
class MediaGateway:
    ...
    def __init__(
        ...
        self.flow_debug: dict[str, tuple[float, dict[int, object], tuple[int, int, int], list[tuple[float, float]], list[object]]] = {}
    ...
    def set_flow_debug(
        ...
        self.flow_debug[camera_id] = (time.monotonic(), states, (current, today, increment), roi or [], events or [])
    ...
    def _decorate(self, camera_id: str, jpeg: bytes) -> bytes:
        ...
        flow_at, flow_states, flow_summary, flow_roi, flow_events = self.flow_debug.get(camera_id, (0.0, {}, None, [], []))
        people = people if now - people_at <= 3.0 else []
        objects = objects if now - objects_at <= 3.0 else []
        safety = safety if now - safety_at <= 3.0 else []
        if now - flow_at > 3.0:
            flow_states, flow_summary, flow_roi, flow_events = {}, None, [], []
        rendered = draw_person_overlays(jpeg, people, flow_states, flow_summary, flow_roi, flow_events)
